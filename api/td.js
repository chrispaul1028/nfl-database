// /api/td — the touchdown board and its graded history, in one function.
//
//   /api/td              → this week's board   (was /api/td-board)
//   /api/td?mode=history → past weeks, graded  (was /api/td-history)
//
// Vercel's Hobby plan caps a deployment at 12 serverless functions and every
// file in api/ counts as one, so these two share a file instead of a folder.
// Existing flags (?debug=1, ?find=, ?season=, ?week=) work exactly as before.
...
const TABLE = "TD Log";
async function getJsonAT(url, headers = {}) { const r = await fetch(url, { headers: { accept: "application/json", ...headers } }); if (!r.ok) throw new Error(`HTTP ${r.status} ${url}`); return r.json(); }
async function historyHandler(req, res) {
  try {
    ...
    do {
      ...
      const d = await getJsonAT(u, { Authorization: `Bearer ${token}` });
      rows.push(...(d.records || []).map((r) => r.fields)); offset = d.offset;
    } while (offset);
    const weeks = [...new Set(rows.map((r) => r.Week))].sort((a, b) => a - b);
    const out = [];
    for (const w of weeks) {
      const ws = await getJsonAT(`${origin}/api/week-stats?season=${season}&week=${w}`).catch(() => null);
...
export default async function handler(req, res) {
  const mode = String(req.query.mode || "board").toLowerCase();
  return mode === "history" ? historyHandler(req, res) : boardHandler(req, res);
}
